  ...
  return (
    <div className="space-y-3">
      ...
      {tab === "generate" ? (
        <>
          <div>
            <label className="text-xs text-muted-foreground block mb-1">{">> DATOS DE ENTRADA:"}</label>
            <textarea
              value={input}
          ...
          {hashes && (
            <div className="text-xs text-muted-foreground border border-border/50 p-2">
              ...
              Entrada: {input.length} bytes | Entropía: ~{Math.min(100, Math.round((new Set(input).size / Math.max(1, input.length)) * 100 * 1.5))}%
            </div>
          )}
        </>
      ) : (
